        ...
        with transaction.atomic():
            for at in attendances:
                if request.method == "POST":
                    at.save_attendance(
                        stt = (datetime.strptime(request.POST[f'stt_time{ at.id }'], '%H:%M')).time(),
                        end = (datetime.strptime(request.POST[f'end_time{ at.id }'], '%H:%M')).time(),
                        break_time = (datetime.strptime(request.POST[f'break_time{ at.id }'], '%H:%M')).time()
                    )
...
def index(request):
    if request.method == "POST":
        ...
        return render(request, 'attendance/index.html',
            {'users':users,'t_user':_u,'t_year':_y,'t_month':_m})